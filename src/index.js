...
const token = "puppet_padlocal_52d8a2c44b6a4bbf95614352ccfa4bae"            // padlocal token
...
bot.on("room-join", async(room, inviteeList, inviter, date) => {
  ...
  console.log("加群⬆️", `room:${roomName}, inviteeList:${inviteeList}, inviter:${inviter}, date:${date}`)
  await room.say(`🎉🎉🎉
欢迎${nameList}加入${roomName},邀请人：${inviter.name()}
进群时间:${moment(date).format('YYYY-MM-DD hh:mm:ss')}`)
})
// 监听退群，只有管理员有权限
bot.on("room-leave", (room, leaverList, remover, date) => {
  console.log('离群通知：', LOGPRE, `on room-leave, room:${room}, leaverList:${leaverList}, remover:${remover}, date:${date}`);
})
